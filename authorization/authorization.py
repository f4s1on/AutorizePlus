...
from helpers.http import get_authorization_header_from_message, get_cookie_header_from_message, isStatusCodesReturned, \
    makeMessage, makeRequest, getResponseBody, IHttpRequestResponseImplementation, getRequestBody,  \
    makeplaintextRequest, makeplaintextReponse
...
def checkBypass(self, oldStatusCode, newStatusCode, oldContent,
    ...
    #正则匹配密文
    if(self.autoDecrypt.isSelected()):
        for k, v in self.badProgrammerDEModel.items():
            continue
        regex = v["response"]
        old = re.search(regex, str(oldContent))
        new = re.match(regex, str(newContent))
        if(old != None and new != None and v["type"] != None):
            old = old.group(1)
            new = new.group(1)
            '''print("old:" + old)
            print("new:" + new)'''
            oldContent = decrypt.decryptJython(old, v["secret1"], v["type"], v["IV1"])
            oldContent = self._helpers.bytesToString(oldContent)
            newContent = decrypt.decryptJython(new, v["secret2"], v["type"], v["IV2"])
            newContent = self._helpers.bytesToString(newContent)
            '''print("olddata:" + oldContent)
            print("newdata:" + newContent)'''
...
def checkAuthorization(self, messageInfo, originalHeaders, checkUnauthorized):
    ...
    #print('request:'+getRequestBody(self, requestResponse))
    #print('response:'+getResponseBody(self, requestResponse))
    if (self.autoDecrypt.isSelected()):
        for k, v in self.badProgrammerDEModel.items():
            continue
        if ( v["type"] != None):
            plaintextMessage = IHttpRequestResponseImplementation(messageInfo.getHttpService(),makeplaintextRequest(self, messageInfo,v["secret1"],v["type"],v["IV1"],v["request"]),makeplaintextReponse(self, messageInfo,v["secret1"],v["type"],v["IV1"],v["response"]))
            NewplaintextMessage = IHttpRequestResponseImplementation(requestResponse.getHttpService(),makeplaintextRequest(self, requestResponse,v["secret2"],v["type"],v["IV2"],v["request"]),makeplaintextReponse(self, requestResponse,v["secret2"],v["type"],v["IV2"],v["response"]))
    else:
        plaintextMessage = messageInfo
        NewplaintextMessage = requestResponse

    #print('request:' + getRequestBody(self, plaintextMessage))


    if checkUnauthorized:
        self._log.add(LogEntry(self.currentRequestNumber,self._callbacks.saveBuffersToTempFiles(NewplaintextMessage), method, self._helpers.analyzeRequest(plaintextMessage).getUrl(),plaintextMessage,impression,self._callbacks.saveBuffersToTempFiles(requestResponseUnauthorized),impressionUnauthorized)) # same requests not include again.
    else:
        self._log.add(LogEntry(self.currentRequestNumber,self._callbacks.saveBuffersToTempFiles(NewplaintextMessage), method, self._helpers.analyzeRequest(plaintextMessage).getUrl(),plaintextMessage,impression,None,"Disabled")) # same requests not include again.
