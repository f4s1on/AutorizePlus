    ...
    if authorizeOrNot and msgBody is not None:
        ...
        if (self.autoDecrypt.isSelected()):
            ...
            for k, v in self.badProgrammerDEModel.items():
                continue
            regex = v["request"]
            data1 = re.match(regex, str(msgBody))
            if(data1 != None and v["type"] != None):
                data2 = data1.group(1)
                data2 = decrypt.decryptJython(data2, v["secret1"], v["type"], v["IV1"])
                msgBody = self._helpers.bytesToString(data2)
        ...
        if (self.autoDecrypt.isSelected()):
            if (v["type"] != None ):
                data2 = decrypt.encryptJython(msgBody, v["secret2"], v["type"], v["IV2"])
                msgBody = self._helpers.bytesToString(data2)
...
def makeplaintextRequest(self, messageInfo, key, mode ,iv,request):
    requestInfo = self._helpers.analyzeRequest(messageInfo)
    headers = requestInfo.getHeaders()
    msgBody = getRequestBody(self,messageInfo)
    msgBody = self._helpers.bytesToString(msgBody)
    regex = request
    enc = re.search(regex, str(msgBody))
    if(enc != None):
        msgBody = enc.group(1)
        msgBody = decrypt.decryptJython(msgBody,key,mode,iv)
    return self._helpers.buildHttpMessage(headers, msgBody)

def makeplaintextReponse(self, messageInfo, key ,mode, iv,response):
    reponseInfo = self._helpers.analyzeResponse(messageInfo.getResponse())
    headers = reponseInfo.getHeaders()
    msgBody = getResponseBody(self,messageInfo)
    msgBody = self._helpers.bytesToString(msgBody)
    regex = response
    dec = re.search(regex, str(msgBody))
    if(dec != None):
        msgBody = dec.group(1)
        msgBody = decrypt.decryptJython(msgBody,key,mode,iv)
    return self._helpers.buildHttpMessage(headers, msgBody)
